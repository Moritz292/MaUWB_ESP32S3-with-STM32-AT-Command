...
#define SWITCH_PIN 41 // switch connected
...
void setup() {
  ...
  pinMode(IN1, OUTPUT);
  pinMode(IN2, OUTPUT);
  pinMode(SWITCH_PIN, INPUT_PULLUP);  // Configure the pin as input with pull-up
...
void loop() {
    ...
    // If the state is stable, check for a press
    if (isButtonPressed(SWITCH_PIN) == true) {
      Serial.println("Button pressed");
      updateDisplay("Button pressed");
    } else {
      Serial.println("Button not pressed");
      updateDisplay("Button not pressed");
    }
